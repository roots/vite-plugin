...
describe("wordpressThemeJson", () => {
    ...
    beforeEach(() => {
        vi.mocked(fs.readFileSync).mockImplementation((path: fs.PathOrFileDescriptor) => {
            if (typeof path === "string" && path.includes("tailwind.config.js")) {
                return `module.exports = ${JSON.stringify(mockTailwindConfig)}`;
            }

            return JSON.stringify(mockBaseThemeJson);
        });

        vi.mocked(path.resolve).mockImplementation((...paths: string[]) => paths.join("/"));
    });
    ...
    `;
        ...
        await expect((plugin.generateBundle as any).call({ emitFile })).rejects.toThrow(
            "Unclosed @theme { block - missing closing brace",
        );
    });
    ...
    it("should preserve existing theme.json settings", () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(JSON.stringify(existingThemeJson));
    ...
    `;
        ...
        expect(fontSizes.some((f: { slug: string }) => f.slug.includes("shadow"))).toBe(false);
    });
    ...
    `;
        ...
        expect(radiusSizes.map((r: { size: string }) => r.size)).toEqual([
            "0.0625rem", // xs
            "0.125rem", // sm
            "0.375rem", // md
            "0.5rem", // lg
    ...
    it("should deduplicate border radius entries with base theme.json", () => {
        const existingThemeJson = {
            settings: {
                border: {
                    radius: true,
                    radiusSizes: [{ name: "Small", slug: "sm", size: "0.125rem" }],
                },
                typography: {
                    fontFamilies: [],
                    fontSizes: [],
    ...
    `;
        ...
        const themeJson = JSON.parse(emitFile.mock.calls[0][0].source);
        const slugs = themeJson.settings.border.radiusSizes.map((r: { slug: string }) => r.slug);
        ...
        const existingThemeJson = {
            settings: {
                border: {
                    ...
                    width: true,
                    radius: true,
                    radiusSizes: [{ name: "Small", slug: "sm", size: "0.25rem" }],
                },
                typography: {
                    fontFamilies: [],
                    fontSizes: [],
    ...
    `;
        ...
        const colorSlugs = themeJson.settings.color.palette.map((c: { slug: string }) => c.slug);
